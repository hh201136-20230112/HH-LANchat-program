...
class Ui_MainWindow(object):
    ...
    # 指令定义区--------------------------
    # 指令操作函数命名格式:commands_函数名
    def commands_help(self):
        self.text_add(self.commands_text[0])
...
"""
    ...
    def text_process(self):
        ...
        if self.join_sever_step == -1:
            # 进入普通消息处理
            if self.is_online and not is_commands:
                self.s.send(bytes(f"{self.user_name}:{strs} - {time.strftime('%Y/%m/%d %H:%M')}", "utf-8"))
            else:
                self.chat_text.append(strs)
                if is_commands:
                    # 处理指令
    ...
    def end_process(self):
        # 线程管理器(把is_running设为False以关闭所有线程)(可以用于启动线程)
        while self.is_running:
            pass
